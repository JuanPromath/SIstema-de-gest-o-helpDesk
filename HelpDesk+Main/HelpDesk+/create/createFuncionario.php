...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if (empty($nome) || empty($cpf) || empty($email) || $cargoId <= 0) {
        ...
        $cpfExists = selectWhere('Funcionario', ['codigo'], "cpf = '".mysqli_real_escape_string($conn, $cpf)."'");
        $emailExists = selectWhere('Funcionario', ['codigo'], "email = '".mysqli_real_escape_string($conn, $email)."'");
        ...
        if ($cpfExists && mysqli_num_rows($cpfExists) > 0) {
            ...
            $result = insert(['nome', 'cpf', 'email', 'id_cargo'], $dadosFuncionario, 'Funcionario');
            
            if ($result) {
                header('Location: ../select/selectFuncionario.php?success=created');
                exit;
  } else {
                $errorMsg = mysqli_error($conn);
                $feedback = '<div class="alert alert-danger fade-in"><i class="bi bi-x-circle"></i> Erro ao cadastrar funcionário. ' . ($errorMsg ? htmlspecialchars($errorMsg) : '') . '</div>';
            }
        }
  }
}
...
<div class="dashboard-header fade-in">
    ...
    <p class="text-muted">Preencha os dados para cadastrar um novo funcionário no sistema</p>
</div>
